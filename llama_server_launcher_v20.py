...
from PyQt6.QtCore import Qt, pyqtSignal, QObject, QByteArray
...
# Flags whose value is a credential. Extend as the launcher gains options.
SECRET_FLAGS = {
    "--api-key": "LLAMA_API_KEY",
}

# Flags worth hoisting into named shell variables for editability.
HOISTED_FLAGS = {
    "-m": "MODEL",
    "--mmproj": "MMPROJ",
}

# Export button stylesheet (consistent with existing top-bar buttons)
EXPORT_BUTTON_QSS = """
    QPushButton { background-color: #FF9800; color: white; border: 1px solid #FF9800;
...
class LlamaServerLauncher(QMainWindow):
    ...
    def _restore_geometry(self):
        """Restore window position/size saved in the config, if available."""
    ...
    def init_variables(self):
        ...
        # Relative path of the model whose settings are currently loaded. Used to
        # keep the dropdown selection consistent with loaded state across filtering.
        self.selected_model_rel = ""
    ...
    def build_top_bar(self, layout: QVBoxLayout):
        ...
        self.export_btn.setToolTip("Export current configuration as a runnable Bash script")
        self.export_btn.clicked.connect(self.export_configuration_script)
        self.export_btn.setStyleSheet(EXPORT_BUTTON_QSS)
        btn_frame.addWidget(self.export_btn)
    ...
    def build_server_path_section(self, layout: QVBoxLayout):
        ...
        self.server_path_edit.setMinimumWidth(300)
        path_layout.addWidget(self.server_path_edit, 1)
    ...
    def build_additional_params_tab(self, parent: QWidget):
        ...
        self.spec_type_combo.setMinimumWidth(150)
        self.spec_type_combo.setToolTip("Speculative decoding type")
        ...
        self.spec_n_max_edit.setMinimumWidth(100)
        self.spec_n_max_edit.setToolTip("Number of tokens to draft for speculative decoding")
        spec_n_max_layout.addWidget(self.spec_n_max_edit)
        spec_n_max_layout.addStretch()
        spec_layout.addLayout(spec_n_max_layout)

        model_draft_layout = QHBoxLayout()
        model_draft_layout.addWidget(QLabel("Draft Model (--model-draft):"))
        self.model_draft_edit = QLineEdit(self.model_draft_default)
        self.model_draft_edit.setToolTip("Draft model for speculative decoding (leave empty for none)")
        model_draft_layout.addWidget(self.model_draft_edit, 1)
        self.browse_model_draft_btn = QPushButton("Browse...")
        self.browse_model_draft_btn.clicked.connect(self.browse_model_draft)
        model_draft_layout.addWidget(self.browse_model_draft_btn)
        spec_layout.addLayout(model_draft_layout)
        ...
        self.cache_k_combo.setMinimumWidth(100)
        self.cache_k_combo.setToolTip("KV cache data type for K (empty = default)")
        cache_k_layout.addWidget(self.cache_k_combo)
        cache_k_layout.addStretch()
        ...
        self.cache_v_combo.setMinimumWidth(100)
        self.cache_v_combo.setToolTip("KV cache data type for V (empty = default)")
        cache_v_layout.addWidget(self.cache_v_combo)
        cache_v_layout.addStretch()
        ...
        self.fit_check = QCheckBox("Fit to device memory (--fit on)")
        self.fit_check.setChecked(self.fit_default)
        self.fit_check.setToolTip("Adjust unset arguments to fit in device memory (default in llama-server is on)")
        flags_layout.addWidget(self.fit_check)
    ...
    def set_status(self, message: str, level: str = "info"):
        """Set the status bar message with a severity icon and color.
        ...
        styles = {
            ...
            "error":   ("\u274C", "#F44336"),       # red
            "warning": ("\u26A0\uFE0F", "#FFB300"),  # amber
            "info":    ("\u2139\uFE0F", "#64B5F6"),  # blue
        }
        icon, color = styles.get(level, styles["info"])
        self.status_label.setText(f"{icon}  {message}")
        self.status_label.setStyleSheet(
            f"QLabel {{ background-color: #2e2e2e; color: {color}; padding: 3px 6px; }}"
        )
    ...
    def create_param_row(self, layout: QVBoxLayout, label: str, default: str, tooltip: str) -> QLineEdit:
        ...
        lbl = QLabel(label)
        lbl.setMinimumWidth(200)  # consistent left column without hard-capping
        lbl.setToolTip(tooltip)
        row_layout.addWidget(lbl)

        edit = QLineEdit(default)
        edit.setMinimumWidth(120)
        edit.setToolTip(tooltip)  # native hover tooltip instead of an inline label
        row_layout.addWidget(edit, 1)  # stretch: field grows to fill available width
    ...
    def save_preset(self):
        ...
        self.save_config()
        self.refresh_preset_list()
        self.set_status(f"Preset '{name}' saved", "success")
    ...
    def load_preset(self):
        ...
        self.set_status(f"Preset '{name}' loaded", "success")
    ...
    def delete_preset(self):
        ...
        self.set_status(f"Preset '{name}' deleted", "success")
    ...
    def copy_preset_for_model(self):
        ...
        model_path = self.get_model_full_path()
        if not model_path:
            self.set_status("No model selected to copy settings from", "warning")
            return
        ...
        self.set_status(f"Settings copied from {self.clipboard_source}", "success")
    ...
    def paste_preset_for_model(self):
        ...
        model_path = self.get_model_full_path()
        if not model_path:
            self.set_status("No model selected to paste settings to", "warning")
            return
        ...
        self.update_command_preview()
        self.set_status(f"Settings pasted from {self.clipboard_source} \u2192 {target_name}", "success")
    ...
    def browse_model_draft(self):
        """Browse for draft model file."""
        path = QFileDialog.getOpenFileName(
            self, "Select Draft Model File",
            self.gguf_dir_edit.text() or os.path.expanduser("~"),
            "GGUF files (*.gguf);;All files (*.*)"
        )[0]
        if path:
            self.model_draft_edit.setText(path)
    ...
    def refresh_gguf_list(self):
        ...
        if self.gguf_files:
            self.set_status(f"Found {len(self.gguf_files)} GGUF file(s)", "info")
        else:
            self.set_status("No GGUF files found in directory", "warning")
    ...
    def filter_gguf_list(self, *args):
        ...
        # Keep the currently-loaded model selected across filtering. If it was
        # filtered out, select nothing (index -1) rather than letting Qt default
        # to item 0 -- otherwise the dropdown would show a different model than
        # the one actually loaded, and picking that item wouldn't fire
        # currentTextChanged (it'd already appear "current"), so the selection
        # would silently fail and the old model's settings would stick.
        self.gguf_combo.setCurrentIndex(self._combo_index_for_rel(self.selected_model_rel))
    ...
    def toggle_favorite(self):
        ...
        model_path = self.get_model_full_path()
        if not model_path:
            self.set_status("No model selected", "warning")
            return

        if model_path in self.favorites:
            del self.favorites[model_path]
            self.set_status(f"Removed from favorites: {os.path.basename(model_path)}", "info")
        else:
            self.favorites[model_path] = {
                "note": "",
                "added": datetime.now().isoformat()
            }
            self.set_status(f"Added to favorites: {os.path.basename(model_path)}", "success")

        self.update_favorite_ui()
        self.save_config()

        # Refresh list to update star markers while preserving selection.
        # The combo items are relative paths (e.g. "subdir/model.gguf"), so we
        # must match on the relative path, not the basename, otherwise models in
        # subdirectories won't match and the combo falls back to the first item.
        gguf_dir = self.gguf_dir_edit.text().strip()
        current_selection = os.path.relpath(model_path, gguf_dir) if gguf_dir else model_path
        self.refresh_gguf_list()
        ...
        for i in range(self.gguf_combo.count()):
            ...
            if actual_name == current_selection:
                self.gguf_combo.setCurrentIndex(i)
                break
    ...
    def save_note(self, event=None):
        ...
        if model_path not in self.favorites:
            if note:
                ...
                self.update_favorite_ui()
                self.set_status("Note saved and added to favorites", "success")
    ...
    def _combo_index_for_rel(self, rel_path: str) -> int:
        """Return the combo index whose (de-starred) text matches rel_path, or -1."""
        if not rel_path:
            return -1
        for i in range(self.gguf_combo.count()):
            text = self.gguf_combo.itemText(i)
            actual = text[2:] if text.startswith("\u2605 ") else text
            if actual == rel_path:
                return i
        return -1
    ...
    def on_gguf_selected(self, selected_text: str):
        ...
        if selected_text:
            ...
            # Record the active model so list filtering keeps it selected
            self.selected_model_rel = selected
    ...
    def get_model_full_path(self) -> str:
        """Get the full path of the active (loaded) model.

        Uses the tracked active model rather than the combo's current text, so
        that filtering the dropdown never changes which model the command uses.
        """
        gguf_dir = self.gguf_dir_edit.text().strip()
        selected = self.selected_model_rel
    ...
    def build_command(self) -> list[str]:
        ...
        model_draft = self.model_draft_edit.text().strip()
        if model_draft:
    ...
    def _model_slug(self) -> str:
        """Return a filesystem-friendly stem for the current model."""
        model_path = self.get_model_full_path()
        if not model_path:
            return "llama"
        base = os.path.basename(model_path)
        name = base[:-5] if base.lower().endswith(".gguf") else base
        return "".join(c if c.isalnum() or c in "_-" else "_" for c in name).strip("_") or "llama"

    def generate_bash_script(self, *, embed_secrets: bool = False) -> str:
        """Render the current configuration as a runnable Bash script.

        Raises ValueError if the current configuration is not launchable.
        """
        cmd = self.build_command()
        model_path = self.get_model_full_path()
        if not model_path:
            raise ValueError("Cannot export: no model selected.")

        binary, args = cmd[0], list(cmd[1:])

        variables: list[tuple[str, str]] = [("LLAMA_SERVER", os.path.abspath(binary))]
        ...
        while i < len(args):
            ...
            value = args[i + 1] if takes_value else None

            if takes_value and flag in SECRET_FLAGS and not embed_secrets:
                env_name = SECRET_FLAGS[flag]
                required_env.append(env_name)
                body.append(f'{flag} "${env_name}"')
            elif takes_value and flag in HOISTED_FLAGS:
                var = HOISTED_FLAGS[flag]
                variables.append((var, os.path.abspath(value)))
                body.append(f'{flag} "${var}"')
            elif takes_value:
                body.append(f"{flag} {shlex.quote(value)}")
            else:
                body.append(flag)

            i += 2 if takes_value else 1

        return self._render_script(variables, required_env, body, embed_secrets)

    def _render_script(self, variables, required_env, body, embed_secrets) -> str:
        """Render the Bash script template."""
        import platform
        ts = datetime.now().astimezone().strftime("%Y-%m-%d %H:%M:%S %Z")
        lines = [
            "#!/usr/bin/env bash",
            f"# Llama Server Launcher {APP_VERSION} — exported configuration",
            f"# Generated: {ts} on {platform.node()} ({platform.system()})",
            "#",
            "# Regenerate from the launcher rather than hand-editing, unless you",
            "# intend this script to diverge from the saved configuration.",
            "#",
            "# Usage:  ./this_script.sh [additional llama-server flags...]",
            "#export CUDA_VISIBLE_DEVICES=0",
        ]
        if embed_secrets:
            lines += [
                "#",
                "# WARNING: this file contains a plaintext credential.",
                "# Do not commit it to version control or share it.",
            ]
        lines += ["", "set -euo pipefail", ""]

        for name, value in variables:
            lines.append(f'{name}="${{{name}:-{shlex.quote(value)}}}"')
        lines.append("")

        for env_name in required_env:
            lines.append(
                f': "${{{env_name}:?Set {env_name} before running this script}}"'
            )
        if required_env:
            lines.append("")

        lines += [
            '[ -x "$LLAMA_SERVER" ] || { echo "Not executable: $LLAMA_SERVER" >&2; exit 1; }',
        ]
        if any(n == "MODEL" for n, _ in variables):
            lines.append(
                '[ -f "$MODEL" ] || { echo "Model not found: $MODEL" >&2; exit 1; }'
            )
        lines.append("")

        lines.append('exec "$LLAMA_SERVER" \\')
        lines += [f"  {arg} \\" for arg in body]
        lines.append('  ${1+"$@"}')
        lines.append("")
        return "\n".join(lines)

    def _write_script(self, path: str, content: str, mode: int) -> None:
        """Atomically write content to path with the given mode."""
        directory = os.path.dirname(path) or "."
        fd, tmp = tempfile.mkstemp(dir=directory, prefix=".export-", suffix=".tmp")
        try:
            # newline="\n" — never emit CRLF; it breaks the shebang under WSL.
    ...
    def export_configuration_script(self) -> None:
        """Prompt for a destination, generate, write, report."""
        try:
        ...
        default_dir = self.config.get("last_export_dir", os.path.expanduser("~"))
        default_name = f"{self._model_slug()}_launch.sh"
        ...
        mode = 0o755
        try:
            self._write_script(path, script, mode)
        except PermissionError as e:
            QMessageBox.critical(self, "Permission Denied", f"Could not write {path}\n\n{e}")
            return
        except OSError as e:
        ...
        self.save_config()
        self.set_status(f"Exported → {path}", "success")
    ...
    def copy_command(self):
        ...
        self.set_status("Command copied to clipboard", "info")
    ...
    def launch_server(self):
        ...
        if self.run_terminal_radio.isChecked():
            try:
                ...
                if launched:
                    self.set_status("Server launched in terminal", "success")
                    self.show_api_info()
                else:
...
"""
        ...
        def copy_base_url():
            ...
            self.set_status("Base URL copied to clipboard", "info")
    ...
    def kill_server(self):
        ...
        if self.server_process is not None:
            ...
            if self.server_process.poll() is not None:
                ...
                self.set_status("Background server had already exited", "warning")
                return
            try:
                self.server_process.terminate()
                self.server_process.wait(timeout=5)
                self.set_status("Background server stopped", "success")
                self._cleanup_server_handles()
                return
            except subprocess.TimeoutExpired:
                self.server_process.kill()
                self._cleanup_server_handles()
                self.set_status("Background server killed", "success")
                return
            except Exception as e:
                self.set_status(f"Error stopping server: {e}", "error")
        ...
        if result == QMessageBox.StandardButton.Yes:
            try:
                ...
                self.set_status("Sent kill signal to all llama-server processes", "success")
            except Exception as e:
    ...
    def apply_settings(self, settings: dict):
        ...
        self.fit_check.setChecked(settings.get("fit", self.fit_default))
        ...
        self.model_draft_edit.setText(settings.get("model_draft", self.model_draft_default))
    ...
    def save_current_settings(self):
        ...
        model_path = self.get_model_full_path()
        if not model_path:
            self.set_status("No model selected to save settings for", "warning")
            return
        ...
        self.save_config()
        self.set_status(f"Settings saved for {os.path.basename(model_path)}", "success")
    ...
    def load_settings_for_model(self, model_path: str):
        ...
        if "model_settings" in self.config and model_path in self.config["model_settings"]:
            ...
            self.set_status(f"Loaded settings for {os.path.basename(model_path)}", "info")
        else:
            self.apply_settings({})
            self.set_status(f"Using default settings for {os.path.basename(model_path)}", "info")
    ...
    def save_config(self):
        ...
        try:
            ...
            # Persist the active model (independent of any active search filter)
            self.config["last_selected_gguf"] = self.selected_model_rel
