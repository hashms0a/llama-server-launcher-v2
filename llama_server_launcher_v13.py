...
class LlamaServerLauncher:
    ...
    def init_variables(self):
        ...
        self.no_mmproj_offload_var = tk.BooleanVar(value=False)
        self.no_mmproj_var = tk.BooleanVar(value=False)
        self.cpu_moe_var = tk.BooleanVar(value=False)
        self.parallel_var = tk.StringVar(value="")
        self.spec_type_enabled_var = tk.BooleanVar(value=False)
        self.spec_type_var = tk.StringVar(value="draft-mtp")
        self.spec_draft_n_max_enabled_var = tk.BooleanVar(value=False)
        self.spec_draft_n_max_var = tk.StringVar(value="2")
    ...
    def build_ui(self):
        ...
        self.build_top_bar(outer_frame)
    ...
    def build_top_bar(self, parent):
        ...
        # Clean dark toolbar
        top_frame = tk.Frame(parent, bg="#363636", padx=10, pady=8)
        top_frame.pack(fill=tk.X)

        # Thin accent line below - green matches Launch button
        accent = tk.Frame(parent, bg="#4CAF50", height=2)
        accent.pack(fill=tk.X)

        # --- Row 1: Run Mode + Control Buttons ---
        row1 = tk.Frame(top_frame, bg="#363636")
        row1.pack(fill=tk.X)

        # Run Mode
        mode_frame = tk.LabelFrame(row1, text="Run Mode", bg="#363636", fg="#aaaaaa",
                                   padx=5, pady=3, relief=tk.GROOVE,
                                   highlightbackground="#555555", highlightthickness=1)
        mode_frame.pack(side=tk.LEFT, padx=(0, 15))

        tk.Radiobutton(mode_frame, text="Run in Terminal", variable=self.run_in_terminal_var, value=True,
                       bg="#363636", fg="#cccccc", selectcolor="#444444",
                       activebackground="#444444", activeforeground="#ffffff",
                       highlightthickness=0).pack(side=tk.LEFT)
        tk.Radiobutton(mode_frame, text="Run in Background", variable=self.run_in_terminal_var, value=False,
                       bg="#363636", fg="#cccccc", selectcolor="#444444",
                       activebackground="#444444", activeforeground="#ffffff",
                       highlightthickness=0).pack(side=tk.LEFT, padx=(10, 0))

        # Control Buttons
        btn_frame = tk.Frame(row1, bg="#363636")
        btn_frame.pack(side=tk.RIGHT)

        # Color-coded control buttons
        self.launch_btn = tk.Button(
            btn_frame, text="▶ Launch Server", command=self.launch_server,
        ...
        self.kill_btn = tk.Button(
            btn_frame, text="■ Kill Server", command=self.kill_server,
            bg="#F44336", fg="white", activebackground="#C62828", activeforeground="white",
            relief=tk.RAISED, padx=12, pady=3, cursor="hand2",
            font=("TkDefaultFont", 9, "bold"),
            highlightbackground="#F44336", highlightthickness=1
        )
        self.kill_btn.pack(side=tk.LEFT, padx=(0, 6))

        self.save_btn = tk.Button(
            btn_frame, text="💾 Save Settings", command=self.save_current_settings,
            bg="#2196F3", fg="white", activebackground="#1565C0", activeforeground="white",
            relief=tk.RAISED, padx=12, pady=3, cursor="hand2",
            font=("TkDefaultFont", 9, "bold"),
            highlightbackground="#2196F3", highlightthickness=1
        )
        self.save_btn.pack(side=tk.LEFT)

        # --- Row 2: Status Indicator - inset look ---
        status_frame = tk.Frame(top_frame, bg="#2a2a2a", padx=1, pady=1)
    ...
    def build_additional_params_tab(self, parent):
        ...
        spec_frame = ttk.LabelFrame(scrollable_frame, text="Speculative Decoding", padding="5")
        spec_frame.pack(fill=tk.X, pady=(0, 10))

        spec_type_row = ttk.Frame(spec_frame)
        spec_type_row.pack(fill=tk.X, pady=2)
        ttk.Checkbutton(spec_type_row, text="--spec-type", variable=self.spec_type_enabled_var).pack(side=tk.LEFT)
        spec_type_combo = ttk.Combobox(
            spec_type_row, textvariable=self.spec_type_var,
            values=["none", "draft-simple", "draft-eagle3", "draft-mtp", "ngram-simple", "ngram-map-k", "ngram-map-k4v", "ngram-mod", "ngram-cache"],
            width=15, state="readonly"
        )
        spec_type_combo.pack(side=tk.LEFT, padx=(5, 10))
        ttk.Label(spec_type_row, text="Speculative decoding type", foreground="gray").pack(side=tk.LEFT)

        spec_n_max_row = ttk.Frame(spec_frame)
        spec_n_max_row.pack(fill=tk.X, pady=2)
        ttk.Checkbutton(spec_n_max_row, text="--spec-draft-n-max", variable=self.spec_draft_n_max_enabled_var).pack(side=tk.LEFT)
        ttk.Entry(spec_n_max_row, textvariable=self.spec_draft_n_max_var, width=15).pack(side=tk.LEFT, padx=(5, 10))
        ttk.Label(spec_n_max_row, text="Number of tokens to draft for speculative decoding", foreground="gray").pack(side=tk.LEFT)
        ...
        ttk.Checkbutton(flags_frame, text="No MMProj Offload (--no-mmproj-offload)", variable=self.no_mmproj_offload_var).pack(anchor=tk.W)
        ttk.Checkbutton(flags_frame, text="No MMProj (--no-mmproj)", variable=self.no_mmproj_var).pack(anchor=tk.W)
        ttk.Checkbutton(flags_frame, text="CPU MoE (--cpu-moe)", variable=self.cpu_moe_var).pack(anchor=tk.W)
    ...
    def build_presets_tab(self, parent):
        ...
        copy_paste_frame = ttk.LabelFrame(parent, text="Copy / Paste Between Models", padding="10")
        copy_paste_frame.pack(fill=tk.X, pady=(0, 10))

        cp_btn_row = ttk.Frame(copy_paste_frame)
        cp_btn_row.pack(fill=tk.X, pady=(0, 5))

        ttk.Button(cp_btn_row, text="Copy Current Model Settings", command=self.copy_preset_for_model).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(cp_btn_row, text="Paste to Current Model", command=self.paste_preset_for_model).pack(side=tk.LEFT, padx=(0, 5))

        self.clipboard_info_var = tk.StringVar(value="No settings copied")
        ttk.Label(copy_paste_frame, textvariable=self.clipboard_info_var, foreground="gray").pack(anchor=tk.W, pady=(5, 0))
    ...
    def copy_preset_for_model(self):
        ...
        model_path = self.get_model_full_path()
        if not model_path:
            self.status_var.set("No model selected to copy settings from")
            return
        ...
        self.clipboard_info_var.set(f"Copied settings from: {self.clipboard_source}")
        self.status_var.set(f"Settings copied from {self.clipboard_source}")

    def paste_preset_for_model(self):
        """Paste previously copied settings onto the current model."""
        if not self.clipboard_settings:
            messagebox.showinfo("Nothing to Paste", "No settings have been copied yet.\n\n"
                                "Select a model, then click 'Copy Current Model Settings' first.")
            return

        model_path = self.get_model_full_path()
        if not model_path:
            self.status_var.set("No model selected to paste settings to")
            return

        target_name = os.path.basename(model_path)
        if not messagebox.askyesno("Paste Settings",
        ...
        self.update_command_preview()
        self.status_var.set(f"Settings pasted from {self.clipboard_source} → {target_name}")
    ...
    def build_command(self) -> list[str]:
        ...
        # Speculative decoding settings
        if self.spec_type_enabled_var.get():
            cmd.extend(["--spec-type", SafeVar.get_str(self.spec_type_var)])

        if self.spec_draft_n_max_enabled_var.get():
            cmd.extend(["--spec-draft-n-max", SafeVar.get_str(self.spec_draft_n_max_var)])
        ...
        if self.no_mmproj_offload_var.get():
            cmd.append("--no-mmproj-offload")

        if self.no_mmproj_var.get():
...
"""
    ...
    def get_current_settings(self) -> dict:
        ...
        return {
            ...
            "no_mmproj_offload": self.no_mmproj_offload_var.get(),
            "no_mmproj": self.no_mmproj_var.get(),
            "cpu_moe": self.cpu_moe_var.get(),
            "custom_args": SafeVar.get_str(self.custom_args_var),
            "spec_type_enabled": self.spec_type_enabled_var.get(),
            "spec_type": SafeVar.get_str(self.spec_type_var),
            "spec_draft_n_max_enabled": self.spec_draft_n_max_enabled_var.get(),
            "spec_draft_n_max": SafeVar.get_str(self.spec_draft_n_max_var),
    ...
    def apply_settings(self, settings: dict):
        ...
        self.no_mmproj_offload_var.set(settings.get("no_mmproj_offload", False))
        self.no_mmproj_var.set(settings.get("no_mmproj", False))
        self.cpu_moe_var.set(settings.get("cpu_moe", False))
        self.custom_args_var.set(settings.get("custom_args", ""))
        self.spec_type_enabled_var.set(settings.get("spec_type_enabled", False))
        self.spec_type_var.set(settings.get("spec_type", "draft-mtp"))
        self.spec_draft_n_max_enabled_var.set(settings.get("spec_draft_n_max_enabled", False))
        self.spec_draft_n_max_var.set(settings.get("spec_draft_n_max", "2"))
